...
def play_audio():
    ...
    if not audio_player.playing:
        thread = threading.Thread(target=lambda: audio_player.play("computer_vision\carrot_audio.mp3"))
        thread.daemon = True
        thread.start()
...
def cleanup():
    ...
    pygame.mixer.quit()
    try:
        wav_path = os.path.splitext("computer_vision/carrot_audio.mp3")[0] + '.wav'
...
# Extract audio from the reference video
    has_audio = extract_audio("computer_vision\carrot.mp4", "computer_vision/carrot_audio.mp3")
    if not has_audio:
